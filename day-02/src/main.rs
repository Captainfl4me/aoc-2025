...
fn part_2(input: &str) -> u64 {
    let res = input.trim_end().split(',').fold(0, |acc, range_str| {
        ...
        for i in range_min..=range_max {
            let i_string = i.to_string();
            let i_vec = i_string.chars().collect::<Vec<char>>();
            let max_size = (i_vec.len() / 2).max(1);

            for chunk_size in 1..=max_size {
                if chunk_size > 0 {
                    let num_iter = i_vec.chunks_exact(chunk_size);
                    if num_iter.remainder().is_empty() && (i_vec.len() > 1) {
                        let num_vec = num_iter.map(|x| x.iter().collect::<String>().parse::<i64>().unwrap()).collect::<Vec<i64>>();
                        let repeat_val = num_vec[0];
                        if num_vec.iter().all(|x| *x == repeat_val) {
                            invalid_id += i;
                            break;
                        }
                    }
...
mod tests_day_02 {
    ...
    fn test_part_2() {
        ...
        assert_eq!(part_2(input), 1188511885);
